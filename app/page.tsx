...
export default function HomePage() {
  ...
  const categoryLabel = filter.category === "all"
    ? "All Links"
    : filter.category.charAt(0).toUpperCase() + filter.category.slice(1) + "s";
  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-[11px] text-zinc-600 font-mono mt-0.5">
              ...
              {filter.query && (
                <span className="text-zinc-500"> · &ldquo;{filter.query}&rdquo;</span>
              )}
            </p>
          </div>
          ...
          {(filter.query || filter.category !== "all") && (
            <button
              onClick={() => setFilter({ query: "", category: "all" })}
      ...
      <BottomBar
        links={links}
        filter={filter}
        onCategoryChange={(cat: LinkCategory | "all") => setFilter({ category: cat })}
